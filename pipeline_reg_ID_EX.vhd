...
entity pipeline_reg_ID_EX is
    port (
        GClock         : in  std_logic;
        GReset         : in  std_logic;
        i_clear        : in  std_logic;  -- '1' = insert NOP bubble (zero all control signals)
        -- Data inputs
...
architecture structural of pipeline_reg_ID_EX is
    ...
    -- All control signals are ANDed with NOT i_clear.
    -- When i_clear='1' every control bit becomes '0' → NOP bubble.
    -- Data signals still propagate (they are don't-cares for a NOP).
...
begin
    ...
    -- ----------------------------------------------------------------
    -- Data registers (always load — data is don't-care during a bubble)
    -- ----------------------------------------------------------------
    reg_pc : reg_8bit
    ...
    -- ----------------------------------------------------------------
    -- MEM control signals (gated by clear)
    -- ----------------------------------------------------------------
    reg_branch : reg_1bit
        port map (GClock => GClock, GReset => GReset,
    ...
    -- ----------------------------------------------------------------
    -- WB control signals (gated by clear)
    -- ----------------------------------------------------------------
